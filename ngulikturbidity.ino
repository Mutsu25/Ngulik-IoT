...
// Sensor suhu masuk ke slot pin digital 4
...
void displayData() {
  ...
  lcd.setCursor(0, 0);
  lcd.print("Suhu : ");
  lcd.print(suhu, 1); // Display temperature with 1 decimal place
  lcd.print(char(223));
  lcd.print("C");
  ...
  lcd.setCursor(0, 1);
  lcd.print("Tds  : ");
  lcd.print(tds); // No decimal places for TDS

  lcd.setCursor(0, 2);
  lcd.print("                    "); // Clear the line by printing spaces
  lcd.setCursor(0, 2);
  lcd.print("PH   : ");
  lcd.print(ph, 2); // Display pH with 2 decimal places

  lcd.setCursor(0, 3);
  lcd.print("                    "); // Clear the line by printing spaces
  lcd.setCursor(0, 3);
  lcd.print("Kekeruhan   : ");
  lcd.print(kekeruhan); // No decimal places for turbidity
}
...
void sensorPh() {
  ...
  if (millis() - samplingTime > samplingInterval) {
    ...
    // Calibration formula for pH sensor
    pHValue = 7 + ((voltage - 1.3) * (7 - 4) / (1.3 - 1.73));
    ph = pHValue;
...
void sensorKekeruhan() {
  ...
  // Calibration formula for turbidity sensor
  if (voltage >= 1.3) {
    kekeruhan = 0; 
  } else if (voltage >= 1.2) {
    kekeruhan = 30 * (1.3 - voltage) / (1.3 - 1.2); 
  } else if (voltage >= 0.7) {
    kekeruhan = 30 + (400 - 30) * (1.2 - voltage) / (1.2 - 0.7); 
  } else if (voltage >= 0.5) {
    kekeruhan = 400 + (500 - 400) * (0.7 - voltage) / (0.7 - 0.5); 
  } else {
    kekeruhan = 500;
  }
